...
### Task 1: Reproducible foundation

**Files:**
- Create: `package.json`, `package-lock.json`, `tsconfig.json`
- Create: `next.config.ts`, `eslint.config.mjs`, `vitest.config.ts`
- Create: `playwright.config.ts`, `.env.example`
- Create: `src/app/layout.tsx`, `src/app/page.tsx`
- Test: `tests/foundation/environment.test.ts`
...
2. Write a failing environment test for required non-secret configuration.
3. Create the smallest application and script surface for lint, type checking,
   unit tests, integration tests, build, and browser tests.
4. Run install, lint, type checking, the failing test, and build.
5. Implement environment parsing and make the foundation test pass.
6. Commit foundation separately from dependency rationale documentation.
...
### Task 4: Persistent conversation service
...
### Task 5: Versioned HTTP and event stream
...
- Create: `src/server/event-stream.ts`
...
### Task 7: Golden conversation interface
...
### Task 8: Browser acceptance with test Agent
