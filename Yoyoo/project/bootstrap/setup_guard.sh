...
YOYOO_BACKUP_SCHEDULE="${YOYOO_BACKUP_SCHEDULE:-daily}" # daily | interval
YOYOO_BACKUP_ON_CALENDAR="${YOYOO_BACKUP_ON_CALENDAR:-*-*-* 03:30:00}"
YOYOO_BACKUP_INTERVAL_MIN="${YOYOO_BACKUP_INTERVAL_MIN:-1440}"
...
export YOYOO_GUARD_ALERT_WEBHOOK='${YOYOO_GUARD_ALERT_WEBHOOK}'
export YOYOO_GUARD_ALERT_CHANNEL='${YOYOO_GUARD_ALERT_CHANNEL}'
export YOYOO_GUARD_ALERT_ON_RECOVER='${YOYOO_GUARD_ALERT_ON_RECOVER}'
export YOYOO_GUARD_ALERT_FEISHU_APP_ID='${YOYOO_GUARD_ALERT_FEISHU_APP_ID}'
export YOYOO_GUARD_ALERT_FEISHU_APP_SECRET='${YOYOO_GUARD_ALERT_FEISHU_APP_SECRET}'
...
[Unit]
Description=Run Yoyoo asset backup (${YOYOO_EMPLOYEE_KEY})

[Timer]
UNIT

if [[ "${YOYOO_BACKUP_SCHEDULE}" == "interval" ]]; then
cat >> "${BACKUP_TIMER}" <<UNIT
OnBootSec=2min
...
cat >> "${BACKUP_TIMER}" <<UNIT
OnCalendar=${YOYOO_BACKUP_ON_CALENDAR}
Persistent=true
Unit=$(basename "${BACKUP_SERVICE}")
UNIT
fi
...
echo "  backup_schedule=${YOYOO_BACKUP_SCHEDULE}"
echo "  backup_on_calendar=${YOYOO_BACKUP_ON_CALENDAR}"
