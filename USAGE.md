...
公网版本已部署到 `https://app.yoyooai.com`，并强制使用账号模式。本机普通模式仍不
要求登录。公网默认不创建 Planner、Builder、Reviewer 演示 Agent，真实 Agent 通过
Agent Gateway 或 AI Card runtime 加入。

- 登录 ID 固定为 `AI_100001`，大小写不敏感；
- 密码由服务器端配置命令设置，仓库没有默认密码；
- 设置页提供“退出登录”，退出会立即撤销当前会话；
- 连续错误登录会进入限流，页面不会暴露账号是否存在；
- 忘记密码时使用首次配置时只显示一次的恢复码走人工恢复流程，不开放短信或邮件找回。

部署和恢复步骤见 `infra/production/README.md`。不要直接把本机 `npm run dev` 或
PostgreSQL 端口暴露到公网。
