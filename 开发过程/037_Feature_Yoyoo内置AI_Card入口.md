...
- AI Card 入口与遗留本地凭据兼容路径统一使用 8 至 128 位密码边界；哈希算法和现有数据库凭据保持不变。
...
- 登录页 UI 单测 9/9 通过，覆盖 SSR 关闭、登录、创建、业务错误、网络错误、回调来源与 state 拦截和 8 位密码边界。
- Yoyoo 全量测试 39 个文件、194 个测试通过。
- Yoyoo PostgreSQL 集成测试 23 个文件通过，其中 131 个测试通过、7 个按既有条件跳过。
- `npm run typecheck`、`npm run lint` 与 `npm run build` 通过。
- Playwright 实测浅色桌面、深色桌面、390px 移动端创建页和身份服务离线错误态，页面无横向溢出，失败后可重试。
- 双服务生产构建浏览器验收通过：同页创建 `AI_100001`、自动进入、第二浏览器登录、稳定 Principal 复用和 YOS Agent 接入均成功。
- 浏览器请求断言确认密码只发送到精确 AI Card Origin，未提交给 Yoyoo。
- 2026-08-15 以 `yoyoo-space:316c5f6` 发布到 `app.yoyooai.com`；容器与公网健康、匿名重定向/401、桌面和 390px 移动端、S256 PKCE、控制台和溢出检查均通过。
- 发布前备份位于 `/opt/yoyoo/backups/embedded-aicard-entry-20260814T152735Z`，数据库 dump、Blob/Nginx 归档和 SHA-256 均已校验；旧镜像保留用于应用级回滚。
- 生产未创建测试身份或映射；真实公网持卡人首次注册和独立安全审查仍未完成。
