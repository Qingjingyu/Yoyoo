    ...
    return (
        <div className="flex mb-4 p-5 bg-n-1 rounded-xl last:mb-0 md:block dark:bg-n-6">
            ...
            <div className="w-[calc(100%-2.5rem)] pl-4 md:w-full md:pl-0 md:pt-4">
                <div>
                    {tr(`socials.item.${item.id}.content`, item.content)}{" "}
                    <a
                    ...
                    {item.tags.map((tag: any, index: number) => (
                        <span className="text-primary-1" key={index}>
                            {" "}#{tr(`socials.tag.${String(tag).toLowerCase()}`, tag)}
                        </span>
                    ))}
                </div>
                ...
                <div className="flex flex-wrap mt-1 -ml-3 md:-mr-2">
                    <button className="btn-stroke-light btn-small ml-3 mt-3">
                        <span>{t("socials.post.shareNow")}</span>
                        <Icon name="external-link" />
                    </button>
                    <button className="btn-stroke-light btn-small ml-3 mt-3">
                        <span>{t("socials.post.edit")}</span>
                        <Icon name="edit" />
                    </button>
                    <button className="btn-stroke-light btn-small ml-3 mt-3">
                        <span>{t("socials.post.newVariation")}</span>
                        <Icon name="plus" />
                    </button>
                    <button className="btn-stroke-light btn-small ml-3 mt-3">
                        <span>{t("socials.post.copy")}</span>
                        <Icon name="copy" />
                    </button>
                </div>
            </div>
