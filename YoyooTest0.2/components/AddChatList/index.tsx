...
const colors = [
    {
        id: "0",
        title: "addChat.color.0",
        color: "#8C6584",
    },
    {
        id: "1",
        title: "addChat.color.1",
        color: "#3E90F0",
    },
    {
        id: "2",
        title: "addChat.color.2",
        color: "#D84C10",
    },
    {
        id: "3",
        title: "addChat.color.3",
        color: "#8E55EA",
    },
    {
        id: "4",
        title: "addChat.color.4",
        color: "#7ECE18",
    },
];
...
const users = [
    {
        id: "0",
        name: "addChat.user.0.name",
        avatar: "/images/avatar.jpg",
        status: "addChat.access.full",
    },
    {
        id: "1",
        name: "addChat.user.1.name",
...
const typesAccess = [
    {
        id: "0",
        title: "addChat.access.full",
    },
    {
        id: "1",
        title: "addChat.access.view",
    },
    {
        id: "2",
        title: "addChat.access.startChat",
    },
];
...
const AddChatList = ({ onCancel }: AddChatListProps) => {
    ...
    return (
        <div className="p-12 lg:px-8 md:pt-6 md:px-5 md:pb-6">
            <div className="mb-8 h4">{t("addChat.title")}</div>
            <div className="relative z-10 flex mb-8 md:block">
                <Field
                    className="grow mr-3 md:mr-0 md:mb-3"
                    label={t("addChat.nameLabel")}
                    placeholder={t("addChat.namePlaceholder")}
                    icon="chat-1"
                    value={name}
                ...
                />
                <Select
                    label={t("addChat.colorLabel")}
                    className="shrink-0 min-w-[14.5rem]"
                    items={colors.map((x) => ({ ...x, title: t(x.title) }))}
                    value={color}
                    onChange={setColor}
                />
            </div>
            <div className="flex mb-2 base2 font-semibold">
                {t("addChat.inviteTeamMember")}
            </div>
            <div className="mb-8 p-5 border border-n-3 rounded-xl md:p-0 md:border-none dark:border-n-5">
                <div className="relative z-5">
                    ...
                    <Select
                        ...
                        items={typesAccess.map((x) => ({ ...x, title: t(x.title) }))}
                        value={typeAccess}
                        onChange={setTypeAccess}
                    />
                </div>
                <div className="mb-5 caption1 text-n-4/50">
                    {t("addChat.onlyInvitedCanAccess")}
                </div>
                {users.map((user) => (
                    <User
                        item={{
            ...
            <div className="flex justify-end">
                ...
                <button className="btn-blue">{t("addChat.addList")}</button>
            </div>
        </div>
    );
};
