...
const ChatItem = ({ item }: ChatItemProps) => {
    ...
    const handleClick = () => {
        ...
        toast((toastItem) => (
            <Notify iconCheck>
                <div className="mr-6 ml-3 h6">{t("rightSidebar.archivedToast")}</div>
                <button
            ...
            <Link className="block" href={item.url}>
                ...
                >
                    <div className="base1 font-semibold dark:text-n-1">
                        {tr(`chatHistory.item.${item.id}.title`, item.title)}
                    </div>
                    <div className="mt-1 truncate caption1 text-n-4">
                        {tr(`chatHistory.item.${item.id}.content`, item.content)}
                    </div>
                    {item.image && (
                    ...
                    <div className="flex justify-between items-center mt-2">
                        ...
                        <div className="caption2 text-n-4/75">
                            {tr(`chatHistory.item.${item.id}.time`, item.time)}
                        </div>
                    </div>
                </div>
            </Link>
