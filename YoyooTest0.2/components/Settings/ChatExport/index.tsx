...
const ChatExport = ({}: ChatExportProps) => {
    ...
    const [checkboxes, setCheckboxes] = useState([
        {
            id: "0",
            titleKey: "settings.chatExport.tag.production",
            color: "#3E90F0",
            isChecked: true,
        },
        {
            id: "1",
            titleKey: "settings.chatExport.tag.favourite",
            color: "#8E55EA",
            isChecked: true,
        },
        {
            id: "2",
            titleKey: "settings.chatExport.tag.archived",
            color: "#8C6584",
            isChecked: false,
        },
        {
            id: "3",
            titleKey: "settings.chatExport.tag.deleted",
            color: "#D84C10",
            isChecked: false,
        },
    ]);
    ...
    return (
        <form className="" action="" onSubmit={(event) => event.preventDefault()}>
            <div className="mb-8 h4">{t("settings.chatExport.title")}</div>
            <div className="flex items-center mb-8">
                <div className="mr-auto base2 text-n-4">
                    {t("settings.chatExport.selectList")}
                </div>
                <Switch
                    value={exportChat}
            ...
            <div className="inline-flex bg-primary-1 rounded-xl md:w-full">
                <button className="btn-blue pr-4 rounded-r-none md:grow">
                    {t("settings.chatExport.downloadConversation")}
                </button>
