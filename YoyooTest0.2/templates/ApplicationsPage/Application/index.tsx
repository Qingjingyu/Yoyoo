...
const Application = ({ item }: ApplicationProps) => {
    const { t } = useLocale();
    const descKey = `applications.item.${item.id}.description`;
    const description = t(descKey) === descKey ? item.description : t(descKey);
        ...
        <div className="flex flex-col w-[calc(33.333%-3.5rem)] mx-7 mt-16 2xl:w-[calc(33.333%-2rem)] 2xl:mx-4 2xl:mt-12 lg:w-[calc(50%-2rem)] md:w-full md:mx-0 md:mt-10">
            <div className="flex items-center mb-auto">
                ...
                <div className="grow">
                    ...
                    <div className="caption1 text-n-4">{description}</div>
                </div>
            </div>
            <button
            ...
            >
                {item.installed ? t("applications.added") : t("applications.add")}
            </button>
        </div>
    );
};
